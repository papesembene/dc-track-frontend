    ...
    <div v-else-if="apprenant" class="space-y-5">
      ...
      <div class="relative overflow-hidden rounded-xl border border-gray-100 bg-white p-5 shadow-sm lg:p-6">
        <div class="relative flex flex-col items-start gap-5 sm:flex-row sm:items-center">
          <div class="flex h-20 w-20 items-center justify-center rounded-xl border-4 border-orange-100 bg-orange-50 text-2xl font-bold text-[#F16E00] sm:h-24 sm:w-24">
            {{ apprenant.initials }}
          </div>
          ...
          <div class="flex-1">
            ...
            <p class="mt-1 text-sm text-gray-500">{{ apprenant.email }}</p>
            <div class="mt-3 flex flex-wrap gap-2">
              <span class="rounded-full bg-orange-50 px-3 py-1 text-xs font-semibold text-[#F16E00]">
                {{ apprenant.promo }}
              </span>
              <span class="rounded-full bg-teal-50 px-3 py-1 text-xs font-semibold text-[#009682]">
                {{ apprenant.referentiel }}
              </span>
            </div>
          </div>

          <div class="shrink-0 rounded-xl border border-orange-100 bg-orange-50 px-6 py-3 text-center">
            <p class="text-3xl font-bold text-[#F16E00]">{{ apprenant.totalSituations }}</p>
            <p class="mt-0.5 text-xs text-gray-500">Situations</p>
          </div>
        </div>
      </div>
